# Caffeを用いた画像分類
サンプルデータは用意していません．各画像データのパスとラベルを記したtrain.txtとval.txtは書き方の参考までにアップしています．

## 0. 各種ファイルについて
caffe/examples/imagenetのファイルを基にしています

## 1. 環境について
ソースコードは，以下のように配置しました．
...
引数に指定した6は"Train loss  vs. Iters"のグラフタイプを作成するオプションである．その他のタイプは`python plot_training_log.py`で確認できる．
